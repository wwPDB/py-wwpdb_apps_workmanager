...
#                                   " and wf_class_id = '%s'",
  ...
  "SELECT_MULTIPLE_MESSAGE_TRACK" : "select dep_set_id, major_issue, last_reminder_sent_date, last_validation_sent_date, last_message_sent_date, " +
    ...
    "SELECT_MULTIPLE_SUBMIT_DATE" : "select dep_set_id, DATE_FORMAT(FROM_UNIXTIME(mtime), '%Y-%m-%d') as submit_date from status.timestamp where event = 'submit2' and dep_set_id in ( '%s' ) order by mtime limit 1" ,
...
#       #
    ...
    def getLastSubmitDate(self, depositionids=None):
        return self._StatusDbApi__getSelectionResult(depositionids, 'SELECT_MULTIPLE_SUBMIT_DATE')
