...
class StatusUpdater(BaseClass):
    ...
    def run(self):
        ...
        self.__updateStatusTables()
        return self._getReturnMessage(self.__entryList, "_StatusUpdater", "Status update for ")
    ...
    def __runSingle(self, entry_id):
        modelFile = self._getExistingArchiveFileWithPickleMessage(entry_id, "model", "pdbx", "latest", "_StatusUpdater")
        if modelFile:
            updatedModelFile = self.__updateModelFile(entry_id, modelFile)
            if updatedModelFile:
                self._copyFileToArchiveDirectory(updatedModelFile, entry_id, "model", "pdbx", "next", "_StatusUpdater")
            #
        #
    ...
    def __updateModelFile(self, entry_id, inputFile):
        ...
        if os.access(os.path.join(self._sessionPath, updatedModelFile), os.F_OK):
            ...
            self._dumpPickle(entry_id + "_StatusUpdater", "Status update failed.")
            return ""
        #
